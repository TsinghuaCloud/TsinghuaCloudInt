...
graph.callLater(function () {
    ...
    edge3.addPathSegement([fromBounds.right, fromBounds.cy - 10]);
    edge3.addPathSegement([fromBounds.right + 15, fromBounds.cy - 10]);
    edge3.addPathSegement([fromBounds.right + 15, fromBounds.cy - 80]);
    edge3.addPathSegement([toBounds.x - 20, fromBounds.cy - 80]);
    edge3.addPathSegement([toBounds.x - 20, toBounds.cy - 5]);
    edge3.addPathSegement([toBounds.x, toBounds.cy - 5]);

    var fromBounds = graph.getUIBounds(edge5.from);
    var toBounds = graph.getUIBounds(edge5.to);
    edge5.addPathSegement([fromBounds.right, fromBounds.cy]);
    edge5.addPathSegement([toBounds.x - 25, fromBounds.cy]);
    edge5.addPathSegement([toBounds.x - 25, toBounds.cy]);
    edge5.addPathSegement([toBounds.x, toBounds.cy]);

    var fromBounds = graph.getUIBounds(edge7.from);
    var toBounds = graph.getUIBounds(edge7.to);
    edge7.addPathSegement([fromBounds.right, fromBounds.cy]);
    edge7.addPathSegement([toBounds.x - 25, fromBounds.cy]);
    edge7.addPathSegement([toBounds.x - 25, toBounds.cy + 5]);
    edge7.addPathSegement([toBounds.x, toBounds.cy + 5]);
