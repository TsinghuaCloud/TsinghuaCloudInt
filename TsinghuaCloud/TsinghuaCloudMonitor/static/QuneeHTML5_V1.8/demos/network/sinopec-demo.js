...
function createGroup(){
    ...
    group.padding = 10;
    group.minSize = {width: 30, height: 30};
    return group;
}
...
function init(){
    ...
    var groupText1 = createGroupText("中国石化内部网络区", 125, 24);
    var groupText2 = createGroupText("远程接入区", 285, 24);
    var groupText3 = createGroupText("安全管理区", 438, 24);

    createEdge(bus1, groupText1);
    createEdge(bus1, groupText2);
    createEdge(bus1, groupText3);

    createNode('中国石化\nInternet节点\n网络', 220, 61, 'Q-cloud', Q.Position.CENTER_MIDDLE).size = {width: 90, height: 55};

    var node1 = createNode('总部核心\n交换机', 187, 144, 'Q-exchanger', Q.Position.LEFT_MIDDLE);
    createAlarmLamp(149, 165, '#0F0');
    var node2 = createNode(null, 263, 144, 'Q-exchanger', Q.Position.RIGHT_MIDDLE);
    createAlarmLamp(292, 153, '#F00', '发现口令猜测攻击事件\n换行也行');

    createEdge(bus1, node1);
    createEdge(bus1, node2);
    createEdge(node1, node2, EDGE_TYPE_FIBER);

    var node3 = createNode('服务器\n交换机', 187, 216, 'Q-exchanger2', Q.Position.LEFT_MIDDLE);
    ...
    var node5 = createNode('交换机', 339, 329, 'Q-exchanger', Q.Position.LEFT_MIDDLE);
    var node6 = createNode('交换机', 399, 329, 'Q-exchanger', Q.Position.RIGHT_MIDDLE);
    ...
    createText('中国石化总部网络区域', 513, 133);
    createText('管理系统网络区域', 549, 216);
